...
Buka terminal — tabel `books` sudah muncul!

![Laravel migrate](../assets/backend/8.png)
...
`Book::all()` mengambil semua data dari tabel.
...
Laravel **otomatis** mem-parse JSON request body. `$request->validate()` juga langsung mengembalikan error 422 jika validasi gagal — tidak perlu try/catch manual!
